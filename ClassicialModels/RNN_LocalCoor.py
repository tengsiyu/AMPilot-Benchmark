...
def train_model(model, X_train, y_train, epochs=300, learning_rate=0.001, device=None):
    ...
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        Xb = X_train.to(device) if device is not None else X_train
        yb = y_train.to(device) if device is not None else y_train
        out = model(Xb)
...
def test_model(model, X_test, y_test, scaler, save_path="Output/RNN_predictions.json", device=None):
    model.eval()
    with torch.no_grad():
...
def main_separate_datasets(train_file_path,
                           test_file_path,
                           model_save_path="Output/RNN_Model.pth",
                           pred_save_path="Output/RNN_predictions.json"):
    np.random.seed(42)
...
if __name__ == "__main__":
    ...
    model_path = "Output/RNN_Model.pth"
    pred_path = "Output/RNN_predictions.json"
