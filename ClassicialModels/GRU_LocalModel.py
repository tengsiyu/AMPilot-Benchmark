...
def test_model(model, X_test, y_test, scaler, save_path="Output/GRU_predictions.json", device=None):
    model.eval()
    with torch.no_grad():
        if device is not None:
...
def main_separate_datasets(train_file_path,
                           test_file_path,
                           model_save_path="Output/GRU_Model.pth",
                           pred_save_path="Output/GRU_predictions.json"):
    np.random.seed(42)
...
if __name__ == "__main__":
    ...
    pred_path = "Output/GRU_predictions.json"
